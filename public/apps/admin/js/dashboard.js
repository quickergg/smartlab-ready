...
(() => {
  ...
  async function updateDashboardCounts() {
    try {
      ...
      const usersPromise = window.SmartLab?.Api?.get ? 
        ...
        fetch('/api/users', { headers: SmartLab.Core.Auth.getAuthHeaders() }).then(res => res.ok ? res.json() : Promise.reject(new Error('Failed to fetch users')));
        
      const pendingPromise = window.SmartLab?.Api?.get ? 
        window.SmartLab.Api.get(appendAcQs('/api/borrowRequests/pending')) : 
        fetch(appendAcQs('/api/borrowRequests/pending'), { headers: SmartLab.Core.Auth.getAuthHeaders() }).then(res => res.ok ? res.json() : Promise.reject(new Error('Failed to fetch pending requests')));
        
      const schedulesPromise = window.SmartLab?.Api?.get ? 
        window.SmartLab.Api.get(appendAcQs('/api/labSchedule')) : 
        fetch(appendAcQs('/api/labSchedule'), { headers: SmartLab.Core.Auth.getAuthHeaders() }).then(res => res.ok ? res.json() : Promise.reject(new Error('Failed to fetch schedules')));
      ...
      const today = new Date().toLocaleDateString('en-CA', { timeZone: 'Asia/Manila' }); // YYYY-MM-DD in PH timezone
  ...
  async function loadRecentActivity() {
    ...
    try {
      ...
      const requestsPromise = window.SmartLab?.Api?.get ? 
        window.SmartLab.Api.get(appendAcQs('/api/borrowRequests?limit=10')) : 
        fetch(appendAcQs('/api/borrowRequests?limit=10'), { headers: SmartLab.Core.Auth.getAuthHeaders() }).then(res => res.ok ? res.json() : Promise.reject(new Error('Failed to fetch requests')));
        
      const usersPromise = window.SmartLab?.Api?.get ? 
        window.SmartLab.Api.get('/api/users?limit=5') : 
        fetch('/api/users?limit=5', { headers: SmartLab.Core.Auth.getAuthHeaders() }).then(res => res.ok ? res.json() : Promise.reject(new Error('Failed to fetch users')));
  ...
  async function loadUserInfo() {
    try {
      // Get user info from session storage (login.js saves: user_id, role, gmail, full_name)
      ...
      const academicResponse = window.SmartLab?.Api?.get ? 
        ...
        await fetch('/api/activeAcademicContext', { headers: SmartLab.Core.Auth.getAuthHeaders() }).then(res => res.ok ? res.json() : null).catch(err => {
          if (window.SmartLab?.Utils?.logError) {
