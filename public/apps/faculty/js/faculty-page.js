...
(() => {
  ...
  // ---------- Helpers ----------
  const formatDate = (d) => SmartLab.Core.Utils.formatDate(d);
  ...
  async function loadEquipment() {
    ...
    try {
      const res = await fetch("/api/equipment", { headers: SmartLab.Core.Auth.getAuthHeaders() });
  ...
  form.addEventListener("submit", async (e) => {
    ...
    if (labChk?.checked) {
      ...
      if (labConflictError && labConflictError.textContent.trim() !== "") {
        ...
        showNotification('Lab schedule conflict detected! Please choose a different time or lab to continue.', 'warning');
        return;
      }
    }
  ...
  async function loadMyRequests() {
    ...
    try {
      const res = await fetch(`/api/myBorrowRequests/my/${userId}`, { headers: SmartLab.Core.Auth.getAuthHeaders() });
      ...
      tbody.innerHTML = rows
        .map((r) => {
          ...
          const timeRange = SmartLab.Core.Utils.formatTimeRange(r.time_start || r.time_of_use, r.time_end);
  ...
  async function loadUserInfo() {
    try {
      ...
      const academicResponse = await fetch('/api/activeAcademicContext', { headers: SmartLab.Core.Auth.getAuthHeaders() });
